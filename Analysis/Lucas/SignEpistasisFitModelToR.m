function  [ G , R]  = SignEpistasisFitModelToR( R , fitnessthreshold   )
%% G = SignEpistasisFitModelToR( R , fitnessthreshold  )
...
for GI = 1:length(G)
    idx = R.VarPos==G.VarPos(GI) & strcmp(R.Perm,G.Perm{GI});
    G.FitImpact{GI} = R.FitImpact(idx);
    G.Seq1{GI}      = R.Seq1(idx);
end
